...
//envia msg para a ttp
function sendInfo(){
    
    if (checkFilds() == 1) { //all info are ok
       window.alert("Parameters OK"); 
...
function proccessAds(info, tag)
{
    ...
    //Captura dos atributos
    for (var i = 1; i <= m; i++ ) {
         selectedad = (Math.floor(Math.random() * (n - 1 + 1)) + 1); //Processo de randomização
         ...
         adsvector[j] = selectedad; //vetor a ser convertido para binario
         if (selectedad == n) {
    ...
    var endereco;
    for(i =0; i<m; i++) {
        endereco = BP_decryptMessage(ads[i], tags[i], 1); //decriptografia dos anúncios selecionados
        endereco = endereco.substring(5, endereco.length);
        window.open("http" + endereco, "_blank");
    }
    
    //geração do vetor binário
    var k = 0;
    for (k=0; k< n; k++) {
        adsvectorbin[k] = 0;
...
function userClass(tipo, email, update, optout, n, m, period, pkx, preferences) {
    ...
    for (var i = 0; i < tree.view.rowCount; i++) {
     ...
     //definicao do pacote de dados para envio a TTP
    structuser[0] = tipo;
    structuser[1] = email;
    structuser[2] = update;
    structuser[3] = optout;
    ...
    tempo_2 = new Date();
    intervalor = tempo_2 - tempo_1;
    alert("tempo chave publica = " + intervalor);
    
    //encriptando chave publica da usuario com a chave publica da TTP usando RSA - versao teste
   /* tempo_1 = new Date();
    ...
    //encriptando preferencias com chave publica da adnet usando Curvas Elipticas
    tempo_1 = new Date();
    var cifro_prefs = BP_encryptMessage(prefs, adn_publicKey);
    tempo_2 = new Date();
    intervalor = tempo_2 - tempo_1;
    alert("tempo preferencias = " + intervalor);
    ...
    tempo_2 = new Date();
    intervalor = tempo_2 - tempo_1;
    alert("tempo assinatura = " + intervalor);
    ...
    //a funcao enviaInfoUsuario() chamada dentro de preferences.xul, apos chamada de userClass, enviar pacote structuser para a TTP
